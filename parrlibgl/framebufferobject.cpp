...
namespace prb {
	void FrameBufferObject::addAttachment(GLenum att, FrameBufferObject::ColorAttachment format) {
		...
		//std::cout << "adding " << util::fboAttachmentToString(att) << "\n";
		...
		attachmentsAdded.push_back(att);
	...
	void FrameBufferObject::init(vec2 size, std::vector<GLenum> attachments, FrameBufferObject::ColorAttachment format) {
		this->sizeX = (int)size.x;
		this->sizeY = (int)size.y;

		glGenFramebuffers(1, &fboID);

		//std::cout << "attachments size " << attachments.size() << "\n";
		for (GLenum a : attachments) {
			addAttachment(a, format);
		}
	}
	//default initialization
	void FrameBufferObject::init() { init(vec2(sizeX, sizeY), { GL_COLOR_ATTACHMENT0, GL_DEPTH_STENCIL_ATTACHMENT }, { GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE, GL_LINEAR,GL_LINEAR, GL_REPEAT, GL_REPEAT }); }

	void FrameBufferObject::bind() {
		util::mStack.push_back(1.f);   // FIX FIRST FRAME GENERATES A DEGENERATE MATRIX STACK FOR SOME REASON
		oldres = cst::res();
	...
	void FrameBufferObject::unbind() {
		...
		cst::res(oldres);
		util::mStack.pop_back();
		//util::popMatrix();
	}

	void FrameBufferObject::drawInside(std::function<void()> const& func) {
		bind();
		func();
		unbind();
	}
	...
	void FrameBufferObject::resize(int sizeX, int sizeY) {
		...
		if (!(this->sizeX == sizeX && this->sizeY == sizeY)) {
			...
			//*this = FrameBufferObject(vec2(sizeX, sizeY), attachmentsAdded, formats[GL_COLOR_ATTACHMENT0] /*MIGHT CREATE AN ERROR IN SOME CASES*/);
		}
	}
	...
	FrameBufferObject::FrameBufferObject(int sizeX, int sizeY, int minFilter, int magFilter) {
		ColorAttachment format; format.minFilter = minFilter; format.magFilter = magFilter;
		init(vec2(sizeX, sizeY), { GL_COLOR_ATTACHMENT0, GL_DEPTH_STENCIL_ATTACHMENT }, format);
	}
	...
	FrameBufferObject::FrameBufferObject(int sizeX, int sizeY, int minFilter, int magFilter, int internalFormat, int format, int type) {
		...
		ColorAttachment tformat; tformat.minFilter = minFilter; tformat.magFilter = magFilter; tformat.internalFormat = internalFormat; tformat.format = format; tformat.type = type;
		init(vec2(sizeX, sizeY), { GL_COLOR_ATTACHMENT0, GL_DEPTH_STENCIL_ATTACHMENT }, tformat);
	}

...
namespace prb {
	...
	protected:
		...
		*/
		...
		std::vector<GLenum> attachmentsAdded;
	...
	public:
		void init();
		void init(vec2 size, std::vector<GLenum> attachments, ColorAttachment format);
		void compCheck();

		void bind();
		void unbind();

		void drawInside(std::function<void()> const& func);

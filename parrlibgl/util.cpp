...
namespace prb {
	namespace util {
		...
		void drawTexture(GLuint const& tex, mat3 const& transform) {
			drawSubTex(tex, vec2(0.f), 1.f, transform);
		}
		...
		Shader blur;
		FrameBufferObject gauss1;
		FrameBufferObject gauss2;
		FrameBufferObject* curGauss = nullptr;
		FrameBufferObject* otherGauss = nullptr;
		FrameBufferObject* applyGauss(std::function<void()> drawFunc, float mult, int iterations, vec2 const& res) {
			if (gauss1.size() != res || gauss2.size() != res) {
				gauss1.resize(res);
				gauss2.resize(res);
			}

			curGauss->bind(); drawFunc(); curGauss->unbind();

			curGauss = (curGauss == &gauss1 ? &gauss2 : &gauss1);
			otherGauss = (curGauss == &gauss1 ? &gauss1 : &gauss2);

			for (int i = 0; i < iterations; i++) {
				blur.use();
				blur.setUniform("windowSize", curGauss->size());
				blur.setUniform("screenSize", curGauss->size());
				blur.setUniform("direction", (i % 2 == 0 ? vec2(1.f, 0.f) : vec2(0.f, 1.f)) * mult);
				curGauss->bind(); util::drawTexture(otherGauss->getPrimaryColorAttachment(), mat3(1.f)); curGauss->unbind();
		...
		FrameBufferObject* applyGauss(FrameBufferObject& target, float mult, int iterations, vec2 const& res) { return applyGauss([&] { util::drawTexture(target.getPrimaryColorAttachment(), mat3(1.f)); }, mult, iterations, res); }
		...
		int writeToBMP(std::string fileName, float* pixels, vec2 const& size) {
			FILE* f;
			unsigned char* img = NULL;

			//int w = pixels[0].size(), h = pixels.size();
			int w = (int)size.x, h = (int)size.y;
			std::cout << "size " << size << " " << w << " " << h << "\n";

			int filesize = w * h * 3 + 54;  //w is your image width, h is image height, both int
			...
			for (int y = 0; y < h; y++) {
				for (int x = 0; x < w; x++) {
					...
					int ti = (y * w + x) * 3;
					img[ti + 2] = (unsigned char)(r);
					img[ti + 1] = (unsigned char)(g);
					img[ti + 0] = (unsigned char)(b);
				}
			}
		...
		int writeToBMP(std::string fileName, std::vector<std::vector<Vector3f>> const& pixels) {
			FILE* f;
			unsigned char* img = NULL;
		...
		void init() {
			blur = Shader("assets/shaders/standard.vert", "assets/shaders/fastGaussian.frag");
			gauss1 = FrameBufferObject(400, 400);
			gauss2 = FrameBufferObject(400, 400);
			curGauss = &gauss1;
			otherGauss = &gauss2;
		}
	}
